...
```python
# It is positive since the Pearson’s r value is +0.50.
# A value of 0.50 indicates a moderate positive correlation between Attack and Defense stats in Pokémon.
```
...
corr = df_pokemon[['HP', 'Attack', 'Defense', 'Sp. Atk', 'Sp. Def', 'Speed']].corr(method='pearson')
...
sns.scatterplot(data=df_pokemon, x='Attack', y='Defense', alpha=0.5)
...
# Optional: Convert types to category for analysis
df_sales_clean['Store_Type'] = df_sales_clean['Store_Type'].astype('category')
df_sales_clean['City_Type'] = df_sales_clean['City_Type'].astype('category')
df_sales_clean['Product_Quality'] = df_sales_clean['Product_Quality'].astype('category')
...
sns.boxplot(data=df_sales_clean, x='Product_Quality', y='Sales')
